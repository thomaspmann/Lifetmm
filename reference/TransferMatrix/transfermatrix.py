...
def I_mat(n1, n2):
    ...
    # Note for normal incidence only -> s and p polarizations equal!
    r = (n1 - n2) / (n1 + n2)
    t = (2 * n1) / (n1 + n2)
...
# calculate incoherent power transmission from air into superstrate
...
for ind, l in enumerate(lambdas):
    ...
    for material in np.arange(1, len(t)):
        ...
        dj = t[material]
        x_indices = np.nonzero(x_mat == material) # indices of points which are in the material layer considered
        x = x_pos[x_indices] - t_cumsum[material - 1] # distance from interface with previous layer
        # Calculate S_Prime (JAP Vol 86 p.487 Eq 12 and 13)
        ...
        # Normalized Electric Field Profile (JAP Vol 86 p.487 Eq 21) (normalised wrt transmission)
...
# Absorption coefficient in 1/cm (JAP Vol 86 p.487 Eq 23)
...
if plotGeneration:
    ...
    Gx = np.sum(Gxl, 2) * lambda_step # Exciton generation rate as a function of position/(sec-cm^3)
    ...
    # calculate Jsc under AM1.5 illumination assuming 100% internal quantum efficiency at all wavelengths (mA/cm^2)
    Jsc = np.sum(Gx) * x_step * 1.0e-7 * q * 1.0e3

    # calculate parasitic absorption
    parasitic_abs = (1.0 - Reflection - Absorption[activeLayer, :])

    print(str(Jsc) + ' (mA/cm^2)\n')
    print('Parasitic Absorption:' + str(parasitic_abs[0]))

pl.show()

# print('Incoming reflection from device (air-glass substrate): ' + str(Reflection[0]))
