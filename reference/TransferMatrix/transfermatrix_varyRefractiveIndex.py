...
layers = ['SiO2', 'Er_Layer', 'Air', 'Epidermis']
thicknesses = [0, 1000, 100, 3000]     # in nm

activeLayer = 1  # indexing starts from 0
layerVaried = 2  # indexing starts from 0

varyStart = 1.2   # starting refractive index of the layer to be varied
varyFinish = 1.6  # final refractive index of the layer to be varied
varyStep = 0.1      # refractive index increments

lambda_start = 1400  # build a wavelength range to calculate over, starting wavelength (nm)
lambda_stop = 1600  # final wavelength (nm)
lambda_step = 10  # wavelength step size

x_step = 10.0  # grid spacing of device cross section simulation in nm
...
varyRefractive = np.arange(varyStart, varyFinish + varyStep, varyStep, float)
...
t = thicknesses
t[0] = 0
activeAbsorption = 0 * varyRefractive

for refrInd, refr in enumerate(varyRefractive):
    # Update the real part of the refractive index for layer being varied for all lambda
    n[layerVaried, :].real = varyRefractive[refrInd]
    ...
    # start looping over wavelengths
    for ind, l in enumerate(lambdas):
        ...
        for material in np.arange(1, len(t)):
            ...
            num = T[ind] * (S_dprime[0, 0] * np.exp(complex(0, -1.0) * xi * (dj - x)) + S_dprime[1, 0] * np.exp(
                complex(0, 1) * xi * (dj - x)))
    ...
    tmp1 = (a[activeLayer, :] * np.real(n[activeLayer, :]))
    ...
    # calculate Jsc
    activeAbsorption[refrInd] = np.sum(Gx) * x_step * 1.0e-7 * q * 1.0e3
...
ax5.set_ylim(ymin=0)
ax5.set_title('Current Density obtained from 100% IQE')
ax5.set_ylabel('Fraction of Absorption Inside Active Layer')
ax5.set_xlabel('Real(Refractive index), n')
ax5.set_ylim(top=ceil(sorted(activeAbsorption)[-1]))
ax5.plot(varyRefractive, activeAbsorption)
fig5.show()
pl.show()
